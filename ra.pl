...
say HTML::Element::->new_from_lol(['a', { name => 'ra' }])->as_HTML;
say HTML::Element::->new_from_lol(['h1', 'Ra'])->as_HTML;
...
for my $item ($idx->look_down(_tag => 'div', id => 'content')->look_down(sub { $_[0]->tag =~ /^(h[34]|a)$/i	})) {
	if ($item->tag eq 'h3') {
		...
		say HTML::Element::->new_from_lol(['a', { name => $item->attr('href') =~ s/[^a-z]//gr }])->as_HTML;
		say HTML::Element::->new_from_lol(['h2', $item->as_trimmed_text])->as_HTML;
...
system qw(ebook-convert ra.html ra.epub --title=Ra --authors=qntm --language=en --input-encoding=utf8 --max-levels=0 --no-default-epub-cover), q{--chapter=//*[(name()='h1' or name()='h2')]}, q{--level1-toc=//*[name()='h1']}, q{--level2-toc=//*[name()='h2']}, q{--level3-toc=//*[name()='h3']};
